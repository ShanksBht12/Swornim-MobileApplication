...
import 'caterer_detail_page.dart';
import 'package:swornim/main.dart';

class CatererListPage extends ConsumerStatefulWidget {
  const CatererListPage({super.key});

  @override
  ConsumerState<CatererListPage> createState() => _CatererListPageState();
}

class _CatererListPageState extends ConsumerState<CatererListPage>
  ...
  Widget build(BuildContext context) {
    ...
    final caterersAsync = ref.watch(serviceProvidersProvider(ServiceProviderType.caterer));

    return Scaffold(
      backgroundColor: theme.scaffoldBackgroundColor,
      body: RefreshIndicator(
        onRefresh: () async {
          ref.invalidate(serviceProvidersProvider(ServiceProviderType.caterer));
        },
        ...
        child: CustomScrollView(
          ...
          slivers: [
            ...
            _buildResultsHeader(theme, caterersAsync),
            _buildCaterersContent(caterersAsync, theme),
                    ...
                    Container(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          hintText: 'Search caterers, specializations, locations...',
  ...
  Widget _buildResultsHeader(ThemeData theme, AsyncValue caterersAsync) {
    return SliverToBoxAdapter(
      child: Container(
          ...
          children: [
            Column(
              ...
              children: [
                ...
                caterersAsync.when(
                  data: (providers) {
                    final caterers = providers.whereType<Caterer>().toList();
                    final filteredCount = _filterAndSortCaterers(caterers).length;
                    return Text(
                      '$filteredCount ${filteredCount == 1 ? 'caterer' : 'caterers'} found',
  ...
  Widget _buildCaterersContent(AsyncValue caterersAsync, ThemeData theme) {
    return caterersAsync.when(
      data: (providers) {
        final caterers = providers.whereType<Caterer>().toList();
        final filteredCaterers = _filterAndSortCaterers(caterers);
        
        if (filteredCaterers.isEmpty) {
          return _buildEmptyState(theme);
        }
        
        return _buildCaterersList(filteredCaterers, theme);
      },
  ...
  Widget _buildCaterersList(List<Caterer> caterers, ThemeData theme) {
    return SliverList(
            ...
            builder: (context, child) {
              ...
              return FadeTransition(
                ...
                child: Transform.translate(
                  ...
                  child: Container(
                    margin: const EdgeInsets.fromLTRB(20, 0, 20, 16),
                    child: _buildCatererCard(caterers[index], theme),
                  ),
                ),
              );
            },
  ...
  Widget _buildCatererCard(Caterer caterer, ThemeData theme) {
    return Container(
      decoration: BoxDecoration(
        color: theme.colorScheme.surface,
        borderRadius: BorderRadius.circular(20),
        ...
        child: InkWell(
          ...
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => CatererDetailPage(catererId: caterer.id),
              ),
            );
          },
          child: Padding(
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Container(
                      ...
                      child: caterer.image.isNotEmpty
                          ? ClipRRect(
                              borderRadius: BorderRadius.circular(16),
                              child: Image.network(
                                caterer.image,
                          ...
                          Row(
                            children: [
                              ...List.generate(5, (index) {
                                return Icon(
                                  index < caterer.rating.floor()
                                      ? Icons.star_rounded
                                      : index < caterer.rating
                ...
                if (caterer.cuisineTypes.isNotEmpty) ...[
                  Wrap(
                    spacing: 6,
                    runSpacing: 6,
                    children: caterer.cuisineTypes.take(4).map((spec) {
  ...
  List<Caterer> _filterAndSortCaterers(List<Caterer> caterers) {
    var filtered = caterers.where((caterer) {
      // Search query filter
      if (_searchQuery.isNotEmpty) {
        final query = _searchQuery.toLowerCase();
        final matchesName = caterer.businessName.toLowerCase().contains(query);
        final matchesLocation = (caterer.location?.name ?? '').toLowerCase().contains(query);
        final matchesSpecialization = caterer.cuisineTypes
      ...
      if (_selectedSpecialization != 'All') {
        if (!caterer.cuisineTypes.contains(_selectedSpecialization)) {
          return false;
        }
      }
      ...
      if (_selectedLocation != 'All') {
        if ((caterer.location?.name ?? '') != _selectedLocation) {
          return false;
        }
      }
      
      // Rating filter
      if (caterer.rating < _minRating) {
        return false;
      }
    ...
    switch (_sortBy) {
      case 'rating':
        ...
        break;
      case 'price_low':
        filtered.sort((a, b) => a.pricePerPerson.compareTo(b.pricePerPerson));
        break;
      case 'price_high':
        filtered.sort((a, b) => b.pricePerPerson.compareTo(a.pricePerPerson));
        break;
      case 'name':
