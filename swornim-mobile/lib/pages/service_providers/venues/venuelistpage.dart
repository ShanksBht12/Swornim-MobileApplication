...
class VenueListPage extends ConsumerStatefulWidget {
  const VenueListPage({super.key});

  @override
  ConsumerState<VenueListPage> createState() => _VenueListPageState();
}

class _VenueListPageState extends ConsumerState<VenueListPage>
  ...
  Widget build(BuildContext context) {
    ...
    final venuesAsync = ref.watch(serviceProvidersProvider(ServiceProviderType.venue));

    return Scaffold(
      backgroundColor: theme.scaffoldBackgroundColor,
      body: RefreshIndicator(
        onRefresh: () async {
          ref.invalidate(serviceProvidersProvider(ServiceProviderType.venue));
        },
        ...
        child: CustomScrollView(
          ...
          slivers: [
            ...
            _buildResultsHeader(theme, venuesAsync),
            _buildVenuesContent(venuesAsync, theme),
                    ...
                    Container(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          hintText: 'Search venues, specializations, locations...',
  ...
  Widget _buildResultsHeader(ThemeData theme, AsyncValue venuesAsync) {
    return SliverToBoxAdapter(
      child: Container(
          ...
          children: [
            Column(
              ...
              children: [
                ...
                venuesAsync.when(
                  data: (providers) {
                    final venues = providers.whereType<Venue>().toList();
                    final filteredCount = _filterAndSortVenues(venues).length;
                    return Text(
                      '$filteredCount ${filteredCount == 1 ? 'venue' : 'venues'} found',
  ...
  Widget _buildVenuesContent(AsyncValue venuesAsync, ThemeData theme) {
    return venuesAsync.when(
      data: (providers) {
        final venues = providers.whereType<Venue>().toList();
        final filteredVenues = _filterAndSortVenues(venues);
        
        if (filteredVenues.isEmpty) {
          return _buildEmptyState(theme);
        }
        
        return _buildVenuesList(filteredVenues, theme);
      },
  ...
  Widget _buildVenuesList(List<Venue> venues, ThemeData theme) {
    return SliverList(
            ...
            builder: (context, child) {
              ...
              return FadeTransition(
                ...
                child: Transform.translate(
                  ...
                  child: Container(
                    margin: const EdgeInsets.fromLTRB(20, 0, 20, 16),
                    child: _buildVenueCard(venues[index], theme),
                  ),
                ),
              );
            },
              ...
              children: [
                ...
                if (venue.venueTypes.isNotEmpty) ...[
                  Wrap(
                    spacing: 6,
                    runSpacing: 6,
                    children: venue.venueTypes.take(4).map((spec) {
  ...
  List<Venue> _filterAndSortVenues(List<Venue> venues) {
    var filtered = venues.where((venue) {
      // Search query filter
      if (_searchQuery.isNotEmpty) {
        final query = _searchQuery.toLowerCase();
        final matchesName = venue.businessName.toLowerCase().contains(query);
        final matchesLocation = (venue.location?.name ?? '').toLowerCase().contains(query);
        final matchesSpecialization = venue.venueTypes
      ...
      if (_selectedSpecialization != 'All') {
        if (!venue.venueTypes.contains(_selectedSpecialization)) {
          return false;
        }
      }
      ...
      if (_selectedLocation != 'All') {
        if ((venue.location?.name ?? '') != _selectedLocation) {
          return false;
        }
      }
      
      // Rating filter
      if (venue.rating < _minRating) {
        return false;
      }
